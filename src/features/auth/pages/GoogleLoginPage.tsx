...
const GoogleLoginPage: React.FC = observer(() => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Call the real Google OAuth login from AuthStore with the container ID
      await authStore.loginWithGoogle('google-signin-container')
  ...
  // Render the Google button when component mounts
  React.useEffect(() => {
    if (!authStore.isLoading) {
      handleGoogleLogin().catch(error => {
        console.log('Auto-render Google button failed:', error)
          ...
          <div>
            ...
            {!authStore.isLoading && (
              <button
                ...
                className="w-full flex justify-center items-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 mt-2"
                style={{ display: 'none' }}
